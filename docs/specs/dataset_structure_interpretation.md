...
```
- 说明
  - 顶层 `Scene_name`：场景大类（例如 `home`、`commercial`）。
  - 二级 `Scene_category`：场景类别（例如室内类型、用途类型）。
...
- 与现有结构的映射
  ...
  - 建议映射：
    - `Scene_name = home`（或按实际数据域划分）
    - `Scene_category = default`（或细分用途）
