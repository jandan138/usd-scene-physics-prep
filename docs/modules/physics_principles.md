...
- 最小物理绑定工具：`docs/modules/usd_physics.md`（对应 `set_physics/pxr_utils/usd_physics.py`）
- 交互预处理：`docs/modules/preprocess_interaction.md`（对应 `set_physics/preprocess_for_interaction.py`）
- 导航预处理：`docs/modules/preprocess_navigation.md`（对应 `set_physics/preprocess_for_navigation.py`）
- 一键 CLI（本篇最相关）：`set_physics/simready.py`

如果你希望进一步“统一输出格式”（导出到 specs 目录结构），可以结合：
- `python -m specs_normalizer`（见 `docs/specs/normalizer_usage.md`）
