...
## 0.5 可控规模化（批处理 + 审核 + 证据链）

如果你要把 C1 从“试点”推进到“全量批处理”，并希望每次执行都有固定的审核 checklist、进度观察方法、证据链文件约定与 ledger（避免重复处理），请配套阅读：

- `docs/operations/asset_dedup_c1_scaling_workflow.md`
