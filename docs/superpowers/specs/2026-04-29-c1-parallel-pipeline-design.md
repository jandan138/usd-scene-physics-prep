...
After Phase 2 completes, the dataset may be missing non-asset directories that were
present in the baseline workspace but not copied during the parallel pipeline:

- `Material/` — contains MDL material definitions referenced by scene files (~29G)
- `Asset_annotation.json` — asset metadata (if applicable)
