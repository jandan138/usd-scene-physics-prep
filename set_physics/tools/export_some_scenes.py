"""
This script is used to export some specific scenes from the source full scenes directory.
Usage:
    python export_some_scenes.py -i {source_full_scenes_parent_dir} -o {target_few_scenes_parent_dir} -n {specific_scene_names...}
    e.g. python export_some_scenes.py -i /ssd/tianshihan/target_69_new -o /ssd/tianshihan/exported_3 -n MWAX5JYKTKJZ2AABAAAAACA8_usd MVUHLWYKTKJ5EAABAAAAADA8_usd MVUHLWYKTKJ5EAABAAAAADI8_usd
"""
    ...
    for material_path in material_bindings:
        ...
        if material_prim:
            for shader in material_prim.GetChildren():
                ...
                material_refs.update([os.path.join("./Materials", texture_path) for texture_path in texture_path_set])
...
def export(src_dir, target_dir, scene_name_list):
    for scene_name in tqdm.tqdm(scene_name_list, desc=f"Exporting scene"):
        scene_dir = os.path.join(src_dir, "scenes", scene_name)
        target_scene_dir = os.path.join(target_dir, "scenes", scene_name)

        usd_files = [f for f in os.listdir(scene_dir) if f.endswith(".usd") or f.endswith(".usda") or f.endswith(".usdz")]
        stage_initial_usd = os.path.join(scene_dir, "start_result_fix.usd")
        if not os.path.exists(stage_initial_usd):
            stage_initial_usd = os.path.join(scene_dir, "start_result_new.usd")
    ...
    # create "Materials" and "models" symlink with stage start_result_dynamic.usd
...
parser.add_argument("-i", "--input", required=True, help="the source parent directory path of scenes, e.g. /ssd/tianshihan/target")
parser.add_argument("-o", "--output", required=True, help="the target parent directory path of scenes, e.g. /ssd/tianshihan/target_new")
