...
def find_usd_files_with_joints(directory):
    usd_files_with_joints = []
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(".usd"):
                ...
                if has_joint:
                    usd_files_with_joints.append(usd_file_path)

    return usd_files_with_joints
...
joint_usd_files = find_usd_files_with_joints(args.search)
if joint_usd_files:
    with open(file=args.out, mode="w") as f:
        for file_path in joint_usd_files:
            print(file_path, file=f)
else:
    print("未找到包含 joint 的 USD 文件")