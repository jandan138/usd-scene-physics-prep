...
def get_candidate_models(models_dir, model_semantic_name):
    ...
    Return:
    - candidate_model_list(absolute paths): a list of candidate models
    """
    candidate_model_list = []
    for root, dirs, files in os.walk(models_dir):
        for file in files:
            if file == "instance.usd" and model_semantic_name in root:
                candidate_model_list.append(os.path.join(root, file))
...
# child.GetRelationship("material:binding").AddTarget(new_material_prim_path)
"""
def update_model_material(model_prim, semantic, candidate_materials):
    for child in model_prim.GetChildren():
        if child.HasRelationship("material:binding"):
            if semantic in ["blanket", "curtain", "wall"]:
                new_material_usd_path = random.choice(list(candidate_materials.keys()))
                material_stage_prim_path = candidate_materials[new_material_usd_path]
            else:
        ...
        update_model_material(child, semantic, candidate_materials)
...
def random_material_pipeline_case(stage, test_semantic, candidate_materials):
    for prim in stage.Traverse():
        if str(prim.GetName()) == test_semantic:
            for model_prim in prim.GetChildren():
                update_model_material(model_prim, test_semantic, candidate_materials)
...
# --- load stage ---
stage_path = "/ssd/tianshihan/target_30_new/scenes/MV4AFHQKTKJZ2AABAAAAAEA8_usd/start_result_fix_bk.usd"
...
# 设置相机的位置和旋转
camera_origin_translate = Gf.Vec3d(-975, -400, 235)
camera_origin_rotate = Gf.Vec3f(70, 0, -270)
...
xform_api.SetRotate(camera_origin_rotate, UsdGeom.XformCommonAPI.RotationOrderXYZ, Usd.TimeCode.Default())
...
random_model_semantics = ["chair", "desk", "couch", "stool", "table"]
models_dir = "/ssd/tianshihan/target_69_new/models"

candidate_models = dict()
for semantic in random_model_semantics:
    if semantic in ["chair", "couch", "desk", "sofachair", "stool", "table"]:
        with open(f"/ssd/tianshihan/test/random_candidates/models/{semantic}.json", "r") as f:
            candidate_models[semantic] = json.load(f)
    else:
        candidate_models[semantic] = get_candidate_models(models_dir, semantic)

random_material_semantics = ["teatable", "wall"]
...
for semantic in random_material_semantics:
    if semantic in ["blanket", "curtain", "wall"]:
        with open(f"/ssd/tianshihan/test/random_candidates/materials/{semantic}.json", "r") as f:
            candidate_materials[semantic] = json.load(f)  # dict
    else:
        candidate_materials[semantic] = get_candidate_materials(materials_dir, semantic)  # list
...
for _ in range(random_times):
    ...
    for i in range(550):
        ...
        if i >= 10:
            get_camera_rgba(camera, f"/ssd/tianshihan/test/rgba_data/scene_1_new/data_{rgba_id}.png")
            rgba_id += 1
