...
def meshes_rotate(mesh_xform, angle_x=0, angle_y=0, angle_z=0):
    pass

# --- load stage ---
# stage_path = "/ssd/tianshihan/target_69_new_bk/scenes/MWAX5JYKTKJZ2AABAAAAACA8_usd/start_result_new.usd"
stage_path = "/ssd/tianshihan/target_30_new/scenes/MV4AFHQKTKJZ2AABAAAAAEA8_usd/start_result_fix.usd"
...
camera_prim.GetPrim().GetAttribute("clippingRange").Set((1, 1000000.0))  # 裁剪范围

# 设置相机的位置和旋转
camera_origin_translate = Gf.Vec3d(-975.0, -400, 235)
...
random_model_semantics = ["chair", "desk", "plant", "couch", "stool", "shoppingtrolley"]
models_dir = "/ssd/tianshihan/target_30_new_bk/models"
materials_dir = "/ssd/tianshihan/target_30_new_bk/Materials"

candidate_models = dict()
for semantic in random_model_semantics:
    candidate_models[semantic] = get_candidate_models(models_dir, semantic)

random_material_semantics = ["table", "teatable", "ground", "wall", "cabinet"]
...
for _ in range(random_times):
    ...
    for i in range(80):
        ...
        if i > 0 and i % 8 == 0:
            get_camera_rgba(camera, f"/ssd/tianshihan/test/rgba_data/scene_1/data_{rgba_id}.png")
            rgba_id += 1
